module Tasks
  class AwsMediaPackageApi
    ...
    def self.create_chanel
      response = @aws_client.create_channel({ id: "screade-mp-chanel-test-1" })
      # Save response.arn
      # Save response.id
      response
    end
    ...
    def self.delete_channel
      @aws_client.delete_channel({ id: "screade-mp-chanel-test-1" })
    end
    
    def self.create_origin_endpoint
      response = @aws_client.create_origin_endpoint({
        channel_id: "screade-mp-chanel-test-1",
